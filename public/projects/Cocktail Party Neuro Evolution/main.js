...
class Agent {
  ...
  resetAgent() {
    ...
    this.interestArray = Array(numberOfAgents).fill(0.8 / numberOfAgents);
    this.popularity = 50;
    this.lastMove = '';
    this.loneliness = 0;
  ...
  speak() {
    ...
    const { opinion, subject } = this.thoughts || {};
  ...
  hear(opinion, speaker, subject) {
    ...
    if (subject === this.id) {
      ...
      this.likeArray[speaker] = Math.max(
        Math.min(this.likeArray[speaker], 1),
        -1,
      );
    } else {
      ...
      this.likeArray[subject] = Math.max(
        Math.min(this.likeArray[subject], 1),
        -1,
      );
...
function stepSim() {
  ...
  agentList.forEach((agent, agentIndex) => {
    agent.update(inputType, function () {
