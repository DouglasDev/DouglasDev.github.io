      ...
      if (centerConfig.type == 'devices') {
        ...
        for (var i = 0; i < lines.length; i++) {
          ...
          ctx.fillText(
            lines[i],
            centerX,
            centerY - lineheight / 2 + i * lineheight,
          );
        }
...
function getDateRangeOfWeek(weekNo) {
  ...
  var rangeIsFrom =
    ...
    d1.getDate() +
    '/' +
    d1.getFullYear().toString().slice(2);
...
let submittedFormsType = 'annually',
  locationType = 'state',
...
function textInLocation(text) {
  for (let i = 0; i < mockData.location[locationType].length; i++) {
    if (mockData.location[locationType][i][0] == text)
      return [true, mockData.location[locationType][i][1]];
  }
  return [false, 0];
}
...
const colors = {
  bar: '#106179',
  selected: '#EF9144',
  pie: ['#02B3D1', '#BBE5F7', '#116189', '#66D9C3', '#42535C', '#0E929F'],
  devices: ['#02B3D1', '#116189'],
};

function generateManyColors(data) {
  let maxLength = Math.max(
    data.location.city.length,
    data.location.state.length,
    data.location.country.length,
  );
  let arr = [];
...
let locationColors = {
  city: generateColorsOfLength(mockData.location.city.length),
...
function typeToMenuLabel(type) {
  ...
  return (
    capital +
    type.slice(1) +
    ' <img src="./img/icon-dropdown-arrow-charts.svg">'
  );
}

function changeSubmittedFormsData(type) {
  submittedFormsType = type;
  document.getElementById('SubmittedFormsDropdownButton').innerHTML =
    typeToMenuLabel(type);
  submittedForms.data.labels = getLabels(
    mockData.submittedForms[submittedFormsType],
  );
  ...
  locationChart.data.datasets[0].backgroundColor =
    locationColors[locationType].slice();
  addCustomLegend('#location-legend', locationChart);
...
function changeSortOrder(data) {
  ...
  document.getElementById('sortLocation').innerHTML =
    sortType +
    '<img src="./img/icon-sort-arrows.svg" alt="" style="padding-left:15px">';
...
function changeLocationData(type) {
  locationType = type;
  document.getElementById('locationDropdownButton').innerHTML =
    typeToMenuLabel(type);
...
let intentOfUseContext = document
  .getElementById('intentOfUse')
  .getContext('2d');
...
function makeLegend(chart) {
  ...
  if (chart.id == 1)
    text.push(
      '<div data-simplebar data-simplebar-auto-hide="false" style="overflow-x: hidden; padding-top=20px">',
    );
  text.push('<ul class="' + chart.id + '-legend">');
  for (var i = 0; i < chart.data.datasets[0].data.length; i++) {
    text.push('<li class="legendEl"><span>');
    text.push(
      '<div class="legendSquare" style="background-color:' +
        chart.data.datasets[0].backgroundColor[i] +
        '"></div><div class="legendSquare" style="background-color:#EF9144; display:none"></div>',
    );
...
function addCustomLegend(legendDiv, chartName) {
  ...
  $(legendDiv).on('click', 'li', function (event) {
    ...
    if ($target.hasClass('active-chart')) {
      ...
      $target.html(
        $target.html() +
          '<img src="./img/icon-close-with-hitArea.svg" style="padding-left:13px" class="close-icon">',
      );
      chartName.data.datasets[0].backgroundColor[$(this).index()] =
        colors.selected;
...
function resetPieChartSelections(dontBlur) {
  ...
  pieCharts.forEach((chart) => {
    if (!chart.wasReset) {
      chart.wasReset = true;
      if (chart == locationChart)
        locationChart.data.datasets[0].backgroundColor =
          locationColors[locationType].slice();
      else if (chart == devicesChart)
        devicesChart.data.datasets[0].backgroundColor = colors.devices.slice();
...
let queryArray = [
  ...
  {
    queryString: '(max-width: 1340px) and (min-width: 1101px)',
    barChartWidth: '520px',
    entries: 8,
  },
  {
    queryString: '(max-width: 1100px) and (min-width: 801px)',
    barChartWidth: '724px',
    entries: 0,
  },
  {
    queryString: '(max-width: 800px) and (min-width: 601px)',
    barChartWidth: '520px',
    entries: 8,
  },
  {
    queryString: '(max-width: 600px) and (min-width: 470px)',
    barChartWidth: '420px',
    entries: 6,
  },
