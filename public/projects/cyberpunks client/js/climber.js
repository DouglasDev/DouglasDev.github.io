...
cyberpunks.Climber = function (game, collisionGroups, size) {
  ...
  this.head_ = this.createBodyPart_('head', headSize, headSize);
  ...
  this.leftFoot_ = this.createBodyPart_('leftFoot', footWidth, footHeight);
  ...
  this.rightFoot_ = this.createBodyPart_('rightFoot', footWidth, footHeight);
  this.leftHand_ = this.createBodyPart_('leftHand', handSize, handSize);
  ...
  this.rightHand_ = this.createBodyPart_('rightHand', handSize, handSize);
...
cyberpunks.Climber.prototype.dragLimbByOtherPlayer = function (
  ...
  this.setLimbProperties_(
    ...
    x,
    y,
    cyberpunks.LimbState.OTHER_DRAGGING,
  );
};
...
cyberpunks.Climber.prototype.dragLimbMyself = function (mouseX, mouseY) {
  ...
  if (cyberpunks.Climber.isBodyPartAt_(this.leftHand_, mouseX, mouseY)) {
    draggableLimb = cyberpunks.DraggableLimb.LEFT_HAND;
  } else if (
    cyberpunks.Climber.isBodyPartAt_(this.rightHand_, mouseX, mouseY)
  ) {
    draggableLimb = cyberpunks.DraggableLimb.RIGHT_HAND;
  } else if (cyberpunks.Climber.isBodyPartAt_(this.leftFoot_, mouseX, mouseY)) {
    draggableLimb = cyberpunks.DraggableLimb.LEFT_FOOT;
  } else if (
    cyberpunks.Climber.isBodyPartAt_(this.rightFoot_, mouseX, mouseY)
  ) {
  ...
  this.forEachDraggableLimb((limbProperties) => {
    ...
    if (limbProperties.state === cyberpunks.LimbState.SELF_DRAGGING) {
      if (this.getForceOnDraggableLimb_(limbProperties.limb) > 1500) {
        ...
        this.numberOfLimbsHoldingOn_() == 0 &&
...
cyberpunks.Climber.prototype.getReportsForServer = function () {
  ...
  this.forEachDraggableLimb((limbProperties) => {
    if (
      limbProperties.state != cyberpunks.LimbState.LOOSE &&
      limbProperties.state != cyberpunks.LimbState.HOLDING &&
...
cyberpunks.Climber.prototype.getForceOnDraggableLimb_ = function (
  ...
  return Math.abs(
    climber.game_.physics.p2.world.constraints[constraintNumber].equations[1]
      .multiplier,
  );
};
...
cyberpunks.Climber.prototype.moveEntireBodyTo = function (
  ...
  this.moveBodyPartTo_(this.upperBody_, climberCenterX, climberCenterY);
  ...
  this.moveBodyPartTo_(
    ...
    climberCenterY +
      this.upperBody_.height / 2 +
      this.lowerBody_.height +
      this.leftUpperLeg_.height / 2,
  );
  this.moveBodyPartTo_(
    this.leftLowerLeg_,
    climberCenterX - this.lowerBody_.width / 2,
    climberCenterY +
      this.upperBody_.height / 2 +
      this.lowerBody_.height +
      this.leftUpperLeg_.height +
      this.leftLowerLeg_.height / 2,
  );
  this.moveBodyPartTo_(
    this.leftFoot_,
    climberCenterX -
      this.lowerBody_.width / 2 +
      this.leftUpperLeg_.width / 2 -
      this.leftFoot_.width / 2,
    climberCenterY +
      this.upperBody_.height / 2 +
      this.lowerBody_.height +
      this.leftUpperLeg_.height +
      this.leftLowerLeg_.height +
      this.leftFoot_.height / 2,
  );
  this.moveBodyPartTo_(
    this.rightUpperLeg_,
    climberCenterX + this.lowerBody_.width / 2,
  ...
  this.moveBodyPartTo_(
    ...
    climberCenterY +
      this.upperBody_.height / 2 +
      this.lowerBody_.height +
      this.rightUpperLeg_.height +
      this.rightLowerLeg_.height / 2,
  );
  this.moveBodyPartTo_(
    this.rightFoot_,
    climberCenterX +
      this.lowerBody_.width / 2 -
      this.rightUpperLeg_.width / 2 +
      this.rightFoot_.width / 2,
    climberCenterY +
      this.upperBody_.height / 2 +
      this.lowerBody_.height +
      this.rightUpperLeg_.height +
      this.rightLowerLeg_.height +
      this.rightFoot_.height / 2,
  );
  this.moveBodyPartTo_(
    this.leftHand_,
    climberCenterX -
      this.upperBody_.width / 2 -
      this.leftUpperArm_.width -
      this.leftLowerArm_.width -
      this.leftHand_.width / 2,
  ...
  this.moveBodyPartTo_(
    ...
    climberCenterX +
      this.upperBody_.width / 2 +
      this.rightUpperArm_.width +
      this.rightLowerArm_.width +
      this.rightHand_.width / 2,
    climberCenterY -
      this.upperBody_.height / 2 +
      this.rightLowerArm_.height / 2,
  );
  this.moveBodyPartTo_(
    this.rightLowerArm_,
    climberCenterX +
...
cyberpunks.Climber.prototype.showDebugGraphics = function () {
  ...
  this.forEachDraggableLimb((limbProperties, bodyPart) => {
    ...
    if (!this.debugForcesText_[limb]) {
      this.debugForcesText_[limb] = this.game_.add.text(0, 0, '', {
        font: '20px Arial',
        align: 'center',
      });
    }
    ...
    var color =
      ((Math.floor(percentRed * 150) + 100) << 16) +
...
cyberpunks.Climber.prototype.enablePhysics_ = function () {
  ...
  this.game_.physics.p2
    .createRevoluteConstraint(
      ...
      this.leftFoot_,
      [
        this.leftFoot_.width / 2 - this.leftLowerLeg_.width / 2,
        -this.leftFoot_.height / 2,
      ],
    )
    .setLimits(-Math.PI / 8, Math.PI / 8);
  this.game_.physics.p2
    .createRevoluteConstraint(
      ...
      this.rightFoot_,
      [
        -this.rightFoot_.width / 2 + this.rightLowerLeg_.width / 2,
        -this.rightFoot_.height / 2,
      ],
    )
    .setLimits(-Math.PI / 8, Math.PI / 8);
  this.game_.physics.p2
    .createRevoluteConstraint(
      ...
      [
        -this.upperBody_.width / 2,
        -this.upperBody_.height / 2 + this.leftUpperArm_.height / 2,
      ],
      this.leftUpperArm_,
  ...
  this.game_.physics.p2
    .createRevoluteConstraint(
      ...
      [
        this.upperBody_.width / 2,
        -this.upperBody_.height / 2 + this.rightUpperArm_.height / 2,
      ],
      this.rightUpperArm_,
...
cyberpunks.Climber.prototype.setLimbProperties_ = function (
  draggableLimb,
  x,
  y,
  state,
...
cyberpunks.Climber.prototype.forEachDraggableLimb = function (callback) {
  for (var draggableLimb in this.draggableLimbProperties_) {
    callback(
      this.draggableLimbProperties_[draggableLimb],
      this.bodyPartForDraggableLimb_(draggableLimb),
    );
  }
};
